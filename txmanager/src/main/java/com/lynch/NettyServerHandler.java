...
public class NettyServerHandler extends ChannelInboundHandlerAdapter {
     ...
     * 创建事务组，并且添加保存事务
     * 需要判断：
     * 如果所有事务都执行了（有结果了，要么回滚，要么提交），且其中有一个
     * 事务需要回滚，那么通知所有客户端进行回滚；
     * 否则，则通知所有客户端进行提交
     *
     * @param ctx
     * @param msg
    ...
    public synchronized void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        String command = jsonObject.getString("command"); //create-创建事务，add-添加事务
        String groupId = jsonObject.getString("groupId");//事务组id
        String transactionType = jsonObject.getString("transactionType");//子事务类型，commit-待提交，rollback-回滚
        ...
        if ("create".equals(command)) {
            ...
            //如果已经接收到结束事务的标记，比较事务是否全部到达，如果已经全部到达则看事务是否需要回滚
